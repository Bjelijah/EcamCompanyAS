...
__android_log_print(ANDROID_LOG_INFO, "fill_context", "0.1");
	...
	jfieldID ice_optID = (*env)->GetFieldID(env,clazz, "ice_opt", "Lcom/howell/utils/StreamReqIceOpt;");
	...
	__android_log_print(ANDROID_LOG_INFO, "jni", "2 playback:%d",c->playback);
	c->beg = jbeg;
	__android_log_print(ANDROID_LOG_INFO, "jni", "3");
	c->end = jend;
	__android_log_print(ANDROID_LOG_INFO, "jni", "4");
	c->re_invite = jre_invite;//false
	__android_log_print(ANDROID_LOG_INFO, "jni", "5 re_invite:%d",c->re_invite);
	c->method_map = jmethod_bitmap;
	__android_log_print(ANDROID_LOG_INFO, "jni", "6 method_map:%d",c->method_map);
	//if (udp_addr != NULL) {
	strncpy(c->udp_addr,cudp_addr,63);
	__android_log_print(ANDROID_LOG_INFO, "jni", "7 c->udp_addr :%s size:%d",c->udp_addr ,strlen(cudp_addr));
	//}
	c->udp_port = judp_port;
__android_log_print(ANDROID_LOG_INFO, "jni", "8 c->udp_port:%d",c->udp_port);
	...
	__android_log_print(ANDROID_LOG_INFO, "jni", "10 comp_cnt:%d",opt->comp_cnt);
	//stun server
	strcpy(opt->stun_addr,cstun_addr);
	__android_log_print(ANDROID_LOG_INFO, "jni", "stun_addr:%s",opt->stun_addr);
	opt->stun_port = jstun_port;
	__android_log_print(ANDROID_LOG_INFO, "jni", "11 stun_port:%d",opt->stun_port);
	//turn server
	if(jturn_addr != NULL){
		const char* cturn_addr = (*env)-> GetStringUTFChars(env,jturn_addr,NULL);
		strcpy(opt->turn_addr,cturn_addr);
		__android_log_print(ANDROID_LOG_INFO, "jni", "turn_addr:%s",opt->turn_addr);
	...
	if(jturn_port != -1){
		...
		__android_log_print(ANDROID_LOG_INFO, "jni", "13 turn_port:%d",opt->turn_port);
	if (jturn_tcp != -1)
	{
		opt->turn_tcp = jturn_tcp;
	}
		__android_log_print(ANDROID_LOG_INFO, "jni", "14 turn_tcp:%d",opt->turn_tcp);
	if(jturn_username != NULL){
		...
		__android_log_print(ANDROID_LOG_INFO, "jni", "15 turn_username:%s",opt->turn_username);
	...
	__android_log_print(ANDROID_LOG_INFO, "jni", "16 turn_password:%s",opt->turn_password);
...
JNIEXPORT int JNICALL Java_com_howell_utils_VideoInviteUtils_setCatchPictureFlag(JNIEnv *env, jclass cls,jlong index,jstring jpath,jint jlength)
{
...
JNIEXPORT void JNICALL Java_com_howell_utils_VideoInviteUtils_joinThread
...
JNIEXPORT jlong JNICALL Java_com_howell_utils_VideoInviteUtils_createHandle
...
JNIEXPORT jstring JNICALL Java_com_howell_utils_VideoInviteUtils_prepareSDP
...
JNIEXPORT jint JNICALL Java_com_howell_utils_VideoInviteUtils_handleRemoteSDP
...
JNIEXPORT jint JNICALL Java_com_howell_utils_VideoInviteUtils_start
...
JNIEXPORT void JNICALL Java_com_howell_utils_VideoInviteUtils_freeHandle
...
JNIEXPORT void JNICALL Java_com_howell_utils_VideoInviteUtils_prepareReplay
(JNIEnv *env, jclass cls, jlong handle){
	//struct StreamResource * res = handle;
	int arr_index = handle;
...
JNIEXPORT jint JNICALL Java_com_howell_utils_VideoInviteUtils_getMethod
(JNIEnv *env, jclass cls,jlong handle){
	int arr_index = handle;
...
JNIEXPORT int JNICALL Java_com_howell_utils_VideoInviteUtils_getStreamCount
(JNIEnv *env, jclass cls,jlong handle){
...
JNIEXPORT void JNICALL Java_com_howell_utils_VideoInviteUtils_playbackPause
(JNIEnv *env, jclass cls,jlong handle,jboolean bPause){
	int arr_index = handle;
	hwplay_pause(res[arr_index]->play_handle,bPause);
}

JNIEXPORT void JNICALL Java_com_howell_utils_VideoInviteUtils_getSdpTime
(JNIEnv *env, jclass cls,jlong handle){
	int arr_index = handle;
	ecam_stream_req_get_sdp_time(res[arr_index]->req,&res[arr_index]->beg_time, &res[arr_index]->end_time);
}

JNIEXPORT long JNICALL Java_com_howell_utils_VideoInviteUtils_getBegSdpTime
(JNIEnv *env, jclass cls,jlong handle){
	int arr_index = handle;
	return res[arr_index]->beg_time;
}

JNIEXPORT long JNICALL Java_com_howell_utils_VideoInviteUtils_getEndSdpTime
(JNIEnv *env, jclass cls,jlong handle){
	int arr_index = handle;
